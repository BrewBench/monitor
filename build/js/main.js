...
angular.module('brewbench-monitor').controller('mainCtrl', function ($scope, $state, $filter, $timeout, $interval, $q, $http, $sce, BrewService) {
  ...
  $scope.downloadAutoSketch = function () {
    ...
    _.each($scope.kettles, function (kettle, i) {
      if (kettle.heater && kettle.heater.sketch || kettle.cooler && kettle.cooler.sketch || kettle.notify.dweet) {
        ...
        //look for triggers
        if (kettle.heater && kettle.heater.sketch) actions += 'trigger("heat","' + kettle.key.replace(/[^a-zA-Z0-9-.]/g, "") + '","' + kettle.heater.pin + '",temp,' + target + ',' + kettle.temp.diff + ',' + !!kettle.notify.slack + ');\n';
        if (kettle.cooler && kettle.cooler.sketch) actions += 'trigger("cool","' + kettle.key.replace(/[^a-zA-Z0-9-.]/g, "") + '","' + kettle.cooler.pin + '",temp,' + target + ',' + kettle.temp.diff + ',' + !!kettle.notify.slack + ');\n';
        if (kettle.notify.dweet) actions += 'dweetAutoCommand("' + kettle.key.replace(/[^a-zA-Z0-9-.]/g, "") + '","' + $scope.settings.recipe.brewer.name + '","' + $scope.settings.recipe.name + '",temp);\n';
      }
    });
    $http.get('assets/arduino/BrewBenchAutoYun/BrewBenchAutoYun.ino').then(function (response) {
      // replace variables
      response.data = response.data.replace('// [actions]', actions).replace('[TPLINK_CONNECTION]', tplink_connection_string).replace('[SLACK_CONNECTION]', $scope.settings.notifications.slack).replace('[FREQUENCY_SECONDS]', $scope.settings.sketches.frequency ? parseInt($scope.settings.sketches.frequency, 10) : 60);
  ...
  $scope.downloadInfluxDBSketch = function () {
    ...
    _.each($scope.kettles, function (kettle, i) {
      ...
      //look for triggers
      if (kettle.heater && kettle.heater.sketch) actions += 'trigger(F("heat"),F("' + kettle.key.replace(/[^a-zA-Z0-9-.]/g, "") + '"),F("' + kettle.heater.pin + '"),temp,' + target + ',' + kettle.temp.diff + ',' + !!kettle.notify.slack + ');\n';
      if (kettle.cooler && kettle.cooler.sketch) actions += 'trigger(F("cool"),F("' + kettle.key.replace(/[^a-zA-Z0-9-.]/g, "") + '"),F("' + kettle.cooler.pin + '"),temp,' + target + ',' + kettle.temp.diff + ',' + !!kettle.notify.slack + ');\n';
      if (kettle.notify.dweet) actions += 'dweetAutoCommand(F("' + kettle.key.replace(/[^a-zA-Z0-9-.]/g, "") + '"),F("' + $scope.settings.recipe.brewer.name + '"),F("' + $scope.settings.recipe.name + '"),temp);\n';
    });
    $http.get('assets/arduino/BrewBenchInfluxDBYun/BrewBenchInfluxDBYun.ino').then(function (response) {
      // replace variables
      response.data = response.data.replace('// [actions]', actions).replace('[INFLUXDB_CONNECTION]', connection_string).replace('[TPLINK_CONNECTION]', tplink_connection_string).replace('[SLACK_CONNECTION]', $scope.settings.notifications.slack).replace('[FREQUENCY_SECONDS]', $scope.settings.sketches.frequency ? parseInt($scope.settings.sketches.frequency, 10) : 60);
      var streamSketch = document.createElement('a');
